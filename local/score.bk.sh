...
cat $data/text | $ref_filtering_cmd | PYTHONIOENCODING=utf-8 local/insert_blank.py > $dir/scoring_kaldi/test_filt.txt || exit 1;
if [ $stage -le 0 ]; then
  ...
  for wip in $(echo $word_ins_penalty | sed 's/,/ /g'); do
    ...
    if $decode_mbr ; then
      $cmd LMWT=$min_lmwt:$max_lmwt $dir/scoring_kaldi/penalty_$wip/log/best_path.LMWT.log \
        ...
        ark:- ark,t:- \| \
        utils/int2sym.pl -f 2- $symtab \| PYTHONIOENCODING=utf-8 local/insert_blank.py \| sort -u \| \
