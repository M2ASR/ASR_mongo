...
samples_per_iter=80000 # this is the target number of egs in each archive of egs
...
cmvn_opts="--norm-means=false --norm-vars=false"  # can be used for specifying CMVN options, if feature type is not lda (if lda,
...
if [ $stage -le 4 ]; then
  ...
  $cmd --max-jobs-run 20 JOB=1:$nj $dir/log/get_egs.JOB.log \
    $get_egs_program --frame-subsampling-factor=$frame_subsampling_factor \
    --length-tolerance=$length_tolerance \
...
if [ $stage -le 5 ]; then
  ...
  if [ $archives_multiple == 1 ]; then # normal case.
    ...
    $cmd --max-jobs-run 5 JOB=1:$num_archives_intermediate $dir/log/shuffle.JOB.log \
